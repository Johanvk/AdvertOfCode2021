"""part 2 of Day 4 Advent of Code 2021"""
        ...
        while x < number_of_bords -1:
            ...
            for y in temp_set:
                ...
                if bingo_number == check_num:
                    ...
                    if bingo == True:
                        last_win = last_bingo(x, number_of_bords, seen_boards)
                        if last_win == True:
                            tot_left =0
...
def last_bingo(board, number_of_bords, seen_boards: Tuple[int, int, Set[int]]) -> bool:
    for x in seen_boards:
        if board in seen_boards:
            break
    else:
        seen_boards.add(board)
    if len(seen_boards) == number_of_bords + 1:
        print("the last winning board")
        return True
                        
                        
def check_bingo(check_set, left_numbers: Tuple[List[int], List[int]]) -> bool:
    for i in range(5):
        for j in range(5):
