...
def parse_diag(diagonal, grit_size, index_set, more_two: Tuple[List[int], int, set, set]) -> Tuple[set, set]:
    number_entries = len(diagonal)
    for x in range(number_entries):
        if diagonal[x][0] > diagonal[x][2] and diagonal[x][1] > diagonal[x][3]:
            range_diag = diagonal[x][0] - diagonal[x][2]
            for y in range(range_diag + 1):
                index = (diagonal[x][1] - y) * grit_size + (diagonal[x][0] - y)
                if index in index_set:
                    more_two.add(index)
                else:
                    index_set.add(index)
        elif diagonal[x][0] > diagonal[x][2] and diagonal[x][1] < diagonal[x][3]:
            range_diag = diagonal[x][0] - diagonal[x][2]
            for y in range(range_diag + 1):
                index = (diagonal[x][1] + y) * grit_size + (diagonal[x][0] - y)
                if index in index_set:
            ...
            range_diag = diagonal[x][2] - diagonal[x][0]
            for y in range(range_diag + 1):
                index = (diagonal[x][1] + y) * grit_size + (diagonal[x][0] + y)
                if index in index_set:
                    more_two.add(index)
...
def main():
    ...
    index_set, more_two = parse_diag(diagonal, grit_size, index_set, more_two)
