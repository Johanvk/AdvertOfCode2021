...
class GetData:
    ...
    def read_data_file(self) -> Tuple[List[List[int]], int, int]:
        ...
        with open("day15-1_input.txt", "r", encoding="utf-8") as input_data:
            for line in input_data:
                ...
                coloms = len(y)
                for node in y:
                    self.node_cost_dict[rows*coloms+position] = node
                    position += 1
                rows += 1
        return rows, coloms, self.node_cost_dict
...
class RiskProcessor:
    ...
    def risk_calculator(self,rows,coloms: int):
        """run dijkstra and return the node with the lowest cost to the right left corner"""
