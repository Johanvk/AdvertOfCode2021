"""part 2 of Day 11 Advent of Code 2021"""
...
def calculate_energy(octopus_dict, number_lines: Tuple[Dict[int,int], int]) -> Tuple[int, int]:
    """ calculate the energy and flashes of the octupus till all octopus flashes together"""
    step_all_flash = 0
    steps = 1000
    flashes = 0
    for x in range(steps):
        ...
        # check when all octopus flashes
        check_flash = list(octopus_dict.values())
        if len(octopus_dict) == check_flash.count(0):
            step_all_flash = x+1
            break
    return flashes, step_all_flash  
    
    
def main():
    """ get input from file and calculate the flashes"""
    octopus_dict, number_lines = read_data_file()
    flashes, step_all_flash = calculate_energy(octopus_dict, number_lines)
    print(f"Total_flash", flashes)
    print(f"all octopuses flashed at step", step_all_flash)
